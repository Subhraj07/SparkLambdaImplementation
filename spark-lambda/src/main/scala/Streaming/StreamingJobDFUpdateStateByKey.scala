...
object StreamingJobDFUpdateStateByKey {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def streamingApp (sc: SparkContext, batchDuration: Duration): StreamingContext= {
      ...
      val statefulActivityByProduct = activittStream.transform(rdd => {
        ...
        var(previoustime,purchase_count,add_to_cart_count,page_view_count) = currentState.getOrElse((System.currentTimeMillis(),0L,0L,0L))
        var result : Option[(Long,Long,Long,Long)] = null

        if (newItemPerKey.isEmpty){
          if(System.currentTimeMillis() - previoustime > 30000 + 4000) // 30000(wait time) 4000(batch interval 4s)
            result = None
          else
            result = Some((previoustime,purchase_count,add_to_cart_count,page_view_count))
        }
        else {
